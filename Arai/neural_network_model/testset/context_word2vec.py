...
try:
    ...
    def train_batch_sg(model, categories, sentences, alpha, work=None):
        """
        ...
        """
        result = 0
        category_vocabs = [model.wv.cvocab[c] for c in categories if w in model.wv.cvocab and
                       model.wv.cvocab[c].sample_int > model.random.rand() * 2**32]
...
def train_sg_pair(model, category, word, context_index, alpha, learn_vectors=True, learn_hidden=True,
...
class Word2Vec(utils.SaveLoad):
    ...
    def build_vocab(self, sentences, keep_raw_vocab=False, trim_rule=None, progress_per=10000, update=False):
        ...
        """
        self.scan_vocab(category_lists, sentences, progress_per=progress_per, trim_rule=trim_rule)  # initial survey
        self.scale_vocab(keep_raw_vocab=keep_raw_vocab, trim_rule=trim_rule, update=update)  # trim by min_count & precalculate downsampling
        self.finalize_vocab(update=update)  # build tables & arrays

    def scan_vocab(self, category_lists, sentences, progress_per=10000, trim_rule=None):
